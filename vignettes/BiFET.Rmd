...
peaks where each row represents the location, read counts,
GC content (proportion of G/C) and type (one of “target”,”background”,”no”)
of the peak. 
...
of each PWM occurrence and its purity score from the algorithm.     
...
```{r, eval=TRUE, echo=TRUE}
# call function to produce TF binding matrix
TFbinding.mat <- bindingTF_per_peak(GRpeaks,GRmotif)

# A vector of read counts per peak.
reads <- GRpeaks$reads 

# A vector of GC contents per peak 
GCcontent <- GRpeaks$GC

# A vector of peak IDs belonging to the target set
targetpeak <- which(GRpeaks$peaktype == "target")

# A vector of peak IDs belonging to the background set
backgroundpeak <- which(GRpeaks$peaktype == "background")

#Finally, the function “calculate_enrich_p” returns a list of 
